...
- **Comments compensating for unclear code** — sometimes the fix is the code, not the comment. Comment quality on its own belongs to `/review-comments`; only raise it here when restructuring the code is what makes the comment unnecessary.
